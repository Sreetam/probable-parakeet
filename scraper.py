...
class Scraper:
    ...
    def get_rss(self, verbose=False, write=False, from_file=False):
        self.rss_links = []
        if from_file==True:
            self.rss = pd.read_csv('./data/rss.csv')
            self.rss_links = list(self.rss['rss-url'])
            return self.rss_links
        rss_list = pd.read_csv('./data/raw_rss_list.csv')
        ...
        if write==True:
            self.rss.to_csv('./data/rss.csv', index=False, quoting=csv.QUOTE_ALL)
        return self.rss_links
    def breaking(self, from_file=False, write=False, verbose=False, mins=15):
        self.breaking_news = []
        if from_file==True:
            self.breaking_news = [list(i) for i in pd.read_csv("./data/breaking.csv").values]
    ...
    def get_news(self, from_file=False, write=False, verbose=False, mins=15):
        self.news = []
        if from_file==True:
            self.news = [list(i) for i in pd.read_csv("./data/news.csv").values]
            return self.news
        for rss in self.rss_links:
            try:
                for i in range(5):
