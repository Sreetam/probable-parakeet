...
class Scraper:
    ...
    def get_rss(self, verbose=False, write=False, from_file=False):
        self.rss_links = []
        if from_file==True:
            self.rss = pd.read_csv('/home/sr/probable-parakeet/data/rss.csv')
            self.rss_links = list(self.rss['rss-url'])
            return self.rss_links
        rss_list = pd.read_csv('/home/sr/probable-parakeet/data/raw_rss_list.csv')
        ...
        if write==True:
            self.rss.to_csv('/home/sr/probable-parakeet/data/rss.csv', index=False, quoting=csv.QUOTE_ALL)
        return self.rss_links
    def breaking(self, from_file=False, write=False, verbose=False, mins=15):
        self.breaking_news = []
        if from_file==True:
            self.breaking_news = [list(i) for i in pd.read_csv("/home/sr/probable-parakeet/data/breaking.csv").values]
        ...
        if write==True:
            ...
            news.to_csv('/home/sr/probable-parakeet/data/breaking.csv', index=False, quoting=csv.QUOTE_ALL)
            news.to_json('/home/sr/probable-parakeet/data/breaking.json', orient="records", indent=4)
        return self.breaking_news
    def get_news(self, from_file=False, write=False, verbose=False, mins=15):
        self.news = []
        if from_file==True:
            self.news = [list(i) for i in pd.read_csv("/home/sr/probable-parakeet/data/news.csv").values]
            return self.news
        for rss in self.rss_links:
            try:
                for i in range(5):
        ...
        if write==True:
            ...
            news = news.sort_values(by='timestamp', ascending=False)
            news.to_csv('/home/sr/probable-parakeet/data/news.csv', index=False, quoting=csv.QUOTE_ALL)
            news.to_json('/home/sr/probable-parakeet/data/news.json', orient="records", indent=4)
        return self.news